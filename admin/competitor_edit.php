...
<tr><td><?= Loc::getMessage('KK_PRICEWATCH_EDIT_TYPE') ?>:</td><td><select name="COLLECTOR_TYPE"<?= $canWrite ? '' : ' disabled' ?>><option value="mock"<?= $values['COLLECTOR_TYPE'] === 'mock' ? ' selected' : '' ?>>mock</option><option value="http"<?= $values['COLLECTOR_TYPE'] === 'http' ? ' selected' : '' ?>>http</option><option value="external"<?= $values['COLLECTOR_TYPE'] === 'external' ? ' selected' : '' ?>>external</option></select></td></tr>
