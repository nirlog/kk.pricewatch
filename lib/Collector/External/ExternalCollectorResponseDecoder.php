...
final class ExternalCollectorResponseDecoder
{
    public function decode(string $json): CollectorResponse
    {
        try {
            ...
            if (!$data instanceof stdClass) throw new InvalidArgumentException();
            if (($data->schema_version ?? null) !== CollectorRequest::SCHEMA_VERSION
                || !is_string($data->request_id ?? null) || trim($data->request_id) === ''
                || !is_bool($data->success ?? null) || !is_array($data->items ?? null)) {
                throw new InvalidArgumentException();
            }
            if ($data->success === false) {
                if ($data->items !== [] || !property_exists($data, 'error')) throw new InvalidArgumentException();
                [$code, $message] = $this->error($data->error);
                return CollectorResponse::failure($data->request_id, $code, $message);
            }
            if (property_exists($data, 'error')) throw new InvalidArgumentException();
            $items = [];
            $seen = [];
            foreach ($data->items as $item) {
                if (!$item instanceof stdClass || !is_string($item->id ?? null) || trim($item->id) === ''
                    || !is_bool($item->success ?? null) || isset($seen[$item->id])) throw new InvalidArgumentException();
                $seen[$item->id] = true;
                if ($item->success) {
                    if (!is_string($item->price ?? null) || !is_string($item->currency ?? null) || property_exists($item, 'error')) throw new InvalidArgumentException();
                    $items[] = CollectorItemResult::success($item->id, $item->price, $item->currency);
                } else {
                    if (!property_exists($item, 'error') || property_exists($item, 'price') || property_exists($item, 'currency')) throw new InvalidArgumentException();
    ...
    {
        if (!$error instanceof stdClass || !is_string($error->code ?? null)
